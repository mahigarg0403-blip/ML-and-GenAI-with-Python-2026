{
  "nbformat": 4,
  "nbformat_minor": 0,
  "metadata": {
    "colab": {
      "provenance": []
    },
    "kernelspec": {
      "name": "python3",
      "display_name": "Python 3"
    },
    "language_info": {
      "name": "python"
    }
  },
  "cells": [
    {
      "cell_type": "code",
      "execution_count": 1,
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "2YnCwcVFS8Sw",
        "outputId": "bc02940b-0798-4915-f05c-94e4daac2b81"
      },
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "Enter length:10\n",
            "Enter breadth:10\n",
            "Area of rectangle= 100.0\n",
            "Enter principal amount:10\n",
            "Enter rate of interest:2\n",
            "Enter time(in years):5\n",
            "Simple interest= 1.0\n",
            "Enter temperature in celsius:80\n",
            "Temperature in fahrenheit= 176.0\n",
            "Enter first number:21\n",
            "Enter second number:22\n",
            "Enter third number:45\n",
            "Average= 29.333333333333332\n",
            "Enter a number:60\n",
            "Square= 3600\n",
            "Cube= 216000\n",
            "Enter first number:21\n",
            "Enter second number:21\n",
            "Before swapping\n",
            "a= 21\n",
            "b= 21\n",
            "After swapping\n",
            "a= 21\n",
            "b= 21\n",
            "Enter student name:saumya\n",
            "Enter roll number:64\n",
            "Enter marks of subject1:21\n",
            "Enter marks of subject2:21\n",
            "Enter marks of subject3:45\n",
            "\n",
            "-----Student Report-----\n",
            "Student Name: saumya\n",
            "Roll Number: 64\n",
            "Marks in Subject1: 21.0\n",
            "Marks in Subject2: 21.0\n",
            "Marks in Subject3: 45.0\n",
            "Total Marks: 87.0\n",
            "Percentage: 29.0\n"
          ]
        }
      ],
      "source": [
        "\n",
        "\n",
        "#Program to Find area of rectangle\n",
        "length=float(input(\"Enter length:\"))\n",
        "breadth=float(input(\"Enter breadth:\"))\n",
        "\n",
        "#Calculating the area\n",
        "area=length*breadth\n",
        "\n",
        "#Display the result\n",
        "print(\"Area of rectangle=\",area)\n",
        "\n",
        "#Program to calculate simple interest\n",
        "p=float(input(\"Enter principal amount:\"))\n",
        "r=float(input(\"Enter rate of interest:\"))\n",
        "t=float(input(\"Enter time(in years):\"))\n",
        "\n",
        "#Calculating the simple interest\n",
        "si=(p*r*t)/100\n",
        "\n",
        "#Display the result\n",
        "print(\"Simple interest=\",si)\n",
        "\n",
        "#Program to convert temperature from celsius to fahrenheit\n",
        "celsius=float(input(\"Enter temperature in celsius:\"))\n",
        "\n",
        "#Formula to convert temperature from celsius to fahrenheit\n",
        "fahrenheit=(celsius*9/5)+32\n",
        "\n",
        "#Display the result\n",
        "print(\"Temperature in fahrenheit=\",fahrenheit)\n",
        "\n",
        "#Program to calculate the average of 3 numbers\n",
        "num1=float(input(\"Enter first number:\"))\n",
        "num2=float(input(\"Enter second number:\"))\n",
        "num3=float(input(\"Enter third number:\"))\n",
        "\n",
        "#Formula to calculate average\n",
        "average=(num1+num2+num3)/3\n",
        "\n",
        "#Display the result\n",
        "print(\"Average=\",average)\n",
        "\n",
        "#Find Square and Cube of a number\n",
        "num=int(input(\"Enter a number:\"))\n",
        "\n",
        "square=num**2\n",
        "cube=num**3\n",
        "\n",
        "#Display the result\n",
        "print(\"Square=\",square)\n",
        "print(\"Cube=\",cube)\n",
        "\n",
        "#Program to swap two numbers without third variable\n",
        "a=int(input(\"Enter first number:\"))\n",
        "b=int(input(\"Enter second number:\"))\n",
        "\n",
        "print(\"Before swapping\")\n",
        "print(\"a=\",a)\n",
        "print(\"b=\",b)\n",
        "\n",
        "a=a+b\n",
        "b=a-b\n",
        "a=a-b\n",
        "\n",
        "print(\"After swapping\")\n",
        "print(\"a=\",a)\n",
        "print(\"b=\",b)\n",
        "\n",
        "#Program to student report\n",
        "#Taking student details\n",
        "name=input(\"Enter student name:\")\n",
        "roll_no=input(\"Enter roll number:\")\n",
        "\n",
        "#Taking the marks of 3 subjects\n",
        "marks1=float(input(\"Enter marks of subject1:\"))\n",
        "marks2=float(input(\"Enter marks of subject2:\"))\n",
        "marks3=float(input(\"Enter marks of subject3:\"))\n",
        "\n",
        "#Calculating total\n",
        "total=marks1+marks2+marks3\n",
        "\n",
        "#Calculating percentage\n",
        "percentage=total/3\n",
        "\n",
        "#Display the report\n",
        "print(\"\\n-----Student Report-----\")\n",
        "print(\"Student Name:\",name)\n",
        "print(\"Roll Number:\",roll_no)\n",
        "print(\"Marks in Subject1:\",marks1)\n",
        "print(\"Marks in Subject2:\",marks2)\n",
        "print(\"Marks in Subject3:\",marks3)\n",
        "print(\"Total Marks:\",total)\n",
        "print(\"Percentage:\",percentage)\n",
        ""
      ]
    }
  ]
}
